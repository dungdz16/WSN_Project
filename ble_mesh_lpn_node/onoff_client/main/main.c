...
// #define CONFIG_BLE_MESH_GATT_PROXY_SERVER
// #define CONFIG_BLE_MESH_FRIEND
...
node_state state;
xTimerHandle xSendOnOffTimer;
xSemaphoreHandle sem_send_onoff;
void send_command_cb(TimerHandle_t xTimer);
...
void app_main(void)
{
    ...
    xSendOnOffTimer = xTimerCreate("Send OnOff Command",
                                    pdMS_TO_TICKS(10000),
                                    true,
                                    0,
                                    send_command_cb);
    if (xSendOnOffTimer != NULL)
    {
        //xTimerStart(xSendOnOffTimer, 10);
    }
    sem_send_onoff = xSemaphoreCreateBinary();
    while(1)
    {
        // if (xSemaphoreTake(sem_send_onoff, 10) == pdTRUE)
        // {
        //     example_ble_mesh_send_gen_onoff_set();
        // }
    }
...
void send_command_cb(TimerHandle_t xTimer)
{
    ESP_LOGI(TAG, "Node OnOff Status: %d", store.onoff);
    xSemaphoreGive(sem_send_onoff);
}

...
static RTC_DATA_ATTR bool is_friendship_established = false;
                                     ...
                                     esp_ble_mesh_msg_ctx_t *ctx, uint8_t onoff)
{
    ...
    if (ESP_BLE_MESH_ADDR_IS_UNICAST(ctx->recv_dst)) {
        ...
        led = &led_state[model->element->element_addr - primary_addr];
        ESP_LOGI(TAG, "Element Address: %04x, Primary Address: %04x", model->element->element_addr, primary_addr);
        board_led_operation(led->pin, onoff);
    }
}
                                              ...
                                              esp_ble_mesh_cfg_server_cb_param_t *param)
{
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT) {
        ...
        case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
            ...
            // if (is_friendship_established == false)
            // {
                // esp_err_t err = esp_ble_mesh_lpn_enable();
                // if (err != ESP_OK)
                // {
                //     ESP_LOGE(TAG, "Fail to enable low power feature");
                // }
                // is_friendship_established = true;
            //}
            //else
            //{
            //}
...
void app_main(void)
{
    ...
    // while (1)
    // {
        // if (is_friendship_established == true)
        // {
